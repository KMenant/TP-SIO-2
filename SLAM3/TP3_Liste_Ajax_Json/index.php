...
$pageInitiale = new page_base("Formulaire");
	...
	else
	{
		...
		$CollSpecialites = $IDconnexion->query("SELECT * FROM specialite");
				...
				<fieldset id="informations_complementaires">
					...
					<div>
						...
						<select id="maForm" name="maForm" >';
							foreach ($CollFormations as $f) {
								$pageInitiale->corps .= '<option value=" ">'.$f->Intitule.'</option>';
							}
					...
					<div>
						...
						<select id="maSpe" name="maSpe" >' ;
							foreach ($CollSpecialites as $s) {
								$pageInitiale->corps .= '<option value=" ">'.$s->Libelle.'</option>';
							}						
						$CollSpecialites -> closeCursor(); // pour libérer la mémoire occupée par le résultat de la requête   
						$CollSpecialites = null ; // pour une autre exécution avec cette variable
